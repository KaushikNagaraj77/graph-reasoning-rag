...
_RELATIONSHIP_SYSTEM = (
    ...
    "EVIDENCE THAT REFUTES A SPECIFIC OPPOSING CLAIM. When a piece of evidence "
    "supports one position AND, by its content, logically REFUTES a specific "
    "claim of the OPPOSING position, propose a CONTRADICTION edge from that "
    "evidence claim to the specific opposing claim it refutes — IN ADDITION to "
    "the support edge it already contributes to its own side. The evidence claim "
    "is the 'from' node and the refuted opposing claim is the 'to' node.\n"
    "Only propose such an edge when the evidence LOGICALLY REFUTES that specific "
    "claim — i.e. if the evidence is true, that opposing claim cannot stand — "
    "NOT merely because the two are on opposite sides of the debate. Topical "
    "adjacency or general disagreement is NOT enough; the refutation must be "
    "direct and specific. Genuine refutations look like these:\n"
    "  - evidence that facing coastlines fit together closely -> CONTRADICTS an "
    "opposing claim that 'the fit is mere coincidence' (a precise, extensive fit "
    "is the direct negation of coincidence).\n"
    "  - identical land/freshwater species on now-separated continents -> "
    "CONTRADICTS 'the continents are permanent/fixed' and CONTRADICTS 'the "
    "distribution is explained by sunken land bridges' where the species (e.g. a "
    "freshwater reptile) could not have used such a bridge.\n"
    "  - matching rock strata / mountain belts across ocean margins -> "
    "CONTRADICTS 'the continents are fixed and have never been joined'.\n"
    "Do NOT invent a refutation that the claim texts do not actually support, and "
    "do NOT propose an evidence->opposing contradiction merely to balance the "
    "graph. Give a specific rationale for each contradiction stating WHICH "
    "opposing claim it refutes and WHY the refutation is logically forced."
)
